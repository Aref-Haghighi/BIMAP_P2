...
# We Convert to float32, scale to [0,1] by min–max; returns zeros if the image is flat
def normalize(x):
    x = np.asarray(x, np.float32)
    mn, mx = float(np.nanmin(x)), float(np.nanmax(x))
    return np.zeros_like(x) if mx <= mn else (x - mn) / (mx - mn)

# If b doesn’t match a in size, resize b to a (bilinear, keep values, anti-alias).
def match_shape(a, b):
    return b if a.shape == b.shape else resize(
        b, a.shape, order=1, preserve_range=True, anti_aliasing=True
    ).astype(np.float32)

# Resizes/normalizes both images to [0,1], then computes PSNR and SSIM with a sensible odd window size
def psnr_ssim(a, b):
    b = match_shape(a, b)
...
# Converts any image (RGB/gray/float) to a nice 8-bit grayscale for BRISQUE:
# If RGB, uses luma weights. 
# Scales to [0,1], upsizes the smaller side to at least 256 px for stable scores. Returns uint8 0–255.
...
# so pyBRISQUE can find svmutil on Windows
try:
...
# Converts grayscale to 3-channel BGR for the library.
# Uses either .get_score(bgr) or a file-based fallback.
# On any error, returns NaN.
try:
...
# --- Result: quality metrics are best-effort; missing libs won’t crash the whole pipeline ----



# ---------- N2V (optional; cached outputs reused if present) ----------
# If N2V isn’t installed, the script explains that you must install it or provide cached *_denoised_*.npy files.
try:
...
# If weights exist (weights_best.h5), load them. Else train a tiny model quickly:
# Randomly sample 64×64 patches from the normalized image. 
# Build N2VConfig (U-Net kernel size, epochs, batch size, learning rate schedule).
...
# Normalize raw image to [0,1], reflect-pad 8 px to avoid border artifacts, predict, unpad, clip to [0,1], and scale back to the original raw intensity range.
# You get a denoised image in raw units (not normalized).
...
# Build RAW-scaled RGB composites (raw & denoised) using a single min–max from all three raw channels.
# Also compute luma (perceptual grayscale) for both.
...
# Otsu threshold on raw luma → binary foreground / background masks.
def fixed_masks_from_raw_luma(raw_luma):
    t = threshold_otsu(raw_luma); bg = raw_luma < t; fg = ~bg
    return fg, bg

# Compute signed (denoised − raw, luma-weighted) and absolute residual images.
...
# FFT → power → radial bin average. Used to compare frequency content (sharpness/noise).
...
# Find the strongest edge via Sobel. - Trace a line centered on that edge (length 160 px).
# Sample intensity profiles on raw & denoised along that line. → For FWHM (sharpness) check.
...
# Standard Full Width at Half Maximum on a 1D profile (returns width in pixels, or NaN).
def fwhm_1d(y):
...
# Map human words (“lower right”) to matplotlib’s loc codes (1–4).
def _loc_to_id(loc_str):
    m = {"upper right":1, "upper left":2, "lower left":3, "lower right":4}
    return m.get(str(loc_str).lower(), 4)


# If pixel size (µm/px) and desired bar length (µm) are valid:
# Convert µm → pixels, 
# Create white AnchoredSizeBar with thickness tied to image height,
# Add label like “2 µm”, Draw it on the supplied axes.
...
# ---------- CSV writer ----------
# If the CSV file is “locked” by another program, it writes to __1, __2, … instead.
...
# ---------- main pipeline ----------
# files_explicit: list of user-selected images (paths).
# force: retrain/redo even if cached denoised *_R/G/B.npy exist.
# pixel_size, bar_length, bar_loc: scale bar options with defaults so the bar always appears.
def run_pipeline(in_dir, out_dir, files_explicit, force=False, model_dir="./models",
    ...
    for i, f in enumerate(files, 1):
        ...
        """" Build paths: <stem>_denoised_R.npy (and G,B).
        If all exist and force=False → np.load them.
        Else:
        If N2V missing → raise helpful error.
        Train or load an N2V model per channel (names include file index and channel tag).
        Predict in raw units and save the *_denoised_*.npy. """
        ...
        # RAW-scale composites → raw/denoised RGB + luma.
        # → foreground/background masks (from raw Otsu).
        ...
        # Metrics
        # PSNR/SSIM per channel (R,G,B) between raw and denoised.
        # BRISQUE/NIQE for raw vs denoised (mean across R,G,B), plus deltas (den − raw).
        ...
        # Residuals / noise / CNR 
        # Signed & absolute residual images (luma).
        # Background noise σ on raw vs denoised; percent change Δσ_bg(%).
        # Gain-match denoised luma using 95th percentile to compute CNR fairly; delta CNR.
        # Edge profile along strongest edge; compute ΔFWHM (den − raw).
        ...
        # (1) RAW vs DENOISED side-by-side. Saved as <stem>_overlay.png
        ...
        # (2) ABS-DIFF (single) (luma) with colorbar. <stem>_difference.png
        ...
        # (3) SIGNED-DIFF (single) (seismic diverging). <stem>_difference_signed.png
        ...
        # (4) BG-only residual - BACKGROUND-ONLY ABS-DIFF + text box showing σ/CNR/FWHM/BRISQUE/NIQE deltas.<stem>_difference_background.png
        ...
        # (5) Triptych with metrics; colorbar only on ABS-DIFF + small metrics box.
        ...
        # (6) Line profile raw vs denoised with ΔFWHM.
        ...
        # (7) Radial power spectrum (log-power vs radius) raw vs denoised.
        ...
        # Row to CSV - Store all computed numbers (PSNR/SSIM, BRISQUE/NIQE raw+den and deltas, σ/CNR/FWHM, etc.) into rows
        rows.append({
            "Image": p.name,
...
# Tiny Tk dialog to select one or more input images; filters common microscopy/image formats.
...
# Tk dialog for choosing an output folder; falls back to the default if canceled.
